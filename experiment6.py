...
def main(fold):
        ...
        for line in f:
            ...
            if counter_word == 0:
                ...
                if line.split()[2] in tag_count.keys():
                    ...
                    tag_count[line.split()[2]] = 1
                if line.split()[1] in word_count.keys():
    ...
    for iwc in range(0, len(statements_train)):
        for jwc in range(0, len(statements_train[iwc])):
            #print "if ", statements_train[iwc][jwc][0]
            if statements_train[iwc][jwc][0] in low_freq_words:
                statements_train[iwc][jwc][0] = "_unk_"
            if str(statements_train[iwc][jwc]) in pairs.keys():
                #print "adding"
                pairs[str(statements_train[iwc][jwc])] += 1
            else:
                pairs[str(statements_train[iwc][jwc])] = 1
    ...
    for index in range(0, len(tags)):
        ...
        # print a[index]
        lambda_[index] = d * (count_non_zeros(a[index]) / float(tag_count[tags[index]]))
        if sum(a[:][index]) != 0:
    ...
    with open(filename_test, "r") as f:
        for line in f:
            ...
            if counter_word == 0:
                ...
                counter_word += 1
                #print lineno
                the_word = line.split()[1]
                if the_word in word_count.keys():
                    statements_test[counter_sentence].append(the_word)
                else:
                    statements_test[counter_sentence].append("_unk_")
                counter_word += 1
    ...
    for i in range(0, len(statements_test)):
        ...
        for tags_index in range(1, len(statements_test[i])):
            sys_file.write(
                str(tags_index) + "\t" + str(statements_test[i][tags_index]) + "\t" + out_list[tags_index - 1] + "\n")
        sys_file.write("\n")
